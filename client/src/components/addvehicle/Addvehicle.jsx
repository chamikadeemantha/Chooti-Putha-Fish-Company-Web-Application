...
const Add = () => {
    ...
    const number_plate_inputHandler = (event) => {
        const { value } = event.target;
    ...
    const validatenumber_plate = (number_plate, setError) => {
        ...
        const letters = number_plate.slice(0, 3); 
        const numbers = number_plate.slice(3); 

        if (!/^[A-Z]{3}$/.test(letters)) {
            setError('First three characters must be upper-case letters.');
    ...
    const driver_phone_inputHandler = (e) => {
        const { name, value } = e.target;
    ...
    const tank_capacity_inputHandler = (e) => {
        const { name, value } = e.target;
    ...
    const validateTankcapacity = (value) => {
        if (!/^\d+(\.\d+)?$/.test(value)) {
            toast.error('\''+value+'\''+' is an invalid capacity.', {
                position: 'top-center',
                autoClose: 1000,
                toastId: 'invalid-TankC-error' // Ensure a unique toastId to prevent duplicates
            });
            return false;
        }
    ...
    const handleInputChangetank_capacity = (event) => {
        const { value } = event.target;
        validateTankcapacity(value, setError);
    };

    /*validation for last location*/

    const last_location_inputHandler = (e) => {
        const { name, value } = e.target;
    ...
    const submitForm = async(e) => {
        ...
        if (!/^[A-Z]{3}$/.test(letters)) {
            toast.error('First two or three characters must be upper-case letters.', {
                position: 'top-center'});
            return ;
        }
        ...
        await axios.post("http://localhost:4000/api/vehicle/createvehicle", vehicle)
        .then((response)=>{
            ...
            <div className="vehicle_container">
                ...
                <div className="vehicle_form-container">
                    ...
                    <div className="vehicle_header">
                        <h2 className="vehicle_form-title">Add New Vehicle</h2>
                    </div>
                    <form className="addvehicleForm" onSubmit={submitForm}>
                       	<div className="vehicle_inputGroup">
                    		<label htmlFor="name">Name</label><br></br>
                    		<input type="text" onChange={(event) => {name_inputHandler(event); handleInputChange(event);}} value={vehicle.name} id="name" name="name" autoComplete='off' placeholder="Enter name"/>
			            </div>
                	    <div className="vehicle_inputGroup">
                    		<label htmlFor="model">Model</label><br></br>
                    		<input type="text" onChange={(event) => {model_inputHandler(event); handleInputChangemodel(event);}} value={vehicle.model} id="model" name="model" autoComplete='off' placeholder="Enter model"/>
                	    </div>
                	    <div className="vehicle_inputGroup">
                    		<label htmlFor="number_plate">Number Plate No</label><br></br>
                    		<input type="text" onChange={(event) => {number_plate_inputHandler(event); handleInputChangenumber_plate(event);}} value={vehicle.number_plate} id="number_plate" name="number_plate" autoComplete='off' placeholder="Enter number plate no"/>
                	    </div>
                	    <div className="vehicle_inputGroup">
                    		<label htmlFor="driver_phone_no">Driver's Phone No</label><br></br>
                    		<input type="text" onChange={(event) => {driver_phone_inputHandler(event); handleInputChangedriver_phone_no(event);}} value={vehicle.driver_phone_no} id="driver_phone_no" name="driver_phone_no" autoComplete='off' placeholder="Enter driver phone no"/>
                	    </div>
                	    ...
                	    <div className="vehicle_inputGroup">
                            ...
                            <div>
                                <input type="radio" id="petrol" name="fuel_type" value="Petrol" onChange={inputHandler} />
                                <label htmlFor="petrol">Petrol</label>
                            </div>

                            <div>
                                <input type="radio" id="diesel" name="fuel_type" value="Diesel" onChange={inputHandler} />
                	    ...
                	    <div className="vehicle_inputGroup">
                    		...
                    		<input type="text" onChange={(event) => {tank_capacity_inputHandler(event); handleInputChangetank_capacity(event);}} value={vehicle.tank_capacity} id="tank_capacity" name="tank_capacity" autoComplete='off' placeholder="Enter tank capacity"/>
                	    </div>
                	    <div className="vehicle_inputGroup">
                    		<label htmlFor="last_location">Last location</label><br></br>
                    		<input type="text" onChange={(event) => {last_location_inputHandler(event); handleInputChangelast_location(event);}} value={vehicle.last_location} id="last_location" name="last_location" autoComplete='off' placeholder="Enter last location"/>
                	    </div>
                	    <div className="vehicle_inputGroup">
                    		<label htmlFor="route">Route</label><br></br>
                    		<input type="text" onChange={(event) => {route_inputHandler(event); handleInputChangeroute(event);}} value={vehicle.route} id="route" name="route" autoComplete='off' placeholder="Enter route"/>
                	    </div>
                        <div className="vehicle_inputGroup">
                            <center>
                                <button type="submit" className='vehicle_submitButton'>ADD Vehicle</button>
                            </center>
                        </div>
                    </form>
                </div>
